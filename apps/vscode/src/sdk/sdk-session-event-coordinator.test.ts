...
describe("SdkSessionEventCoordinator", () => {
	...
	it("ignores events from closed/replaced sessions", async () => {
		// Cline Cubed: sessions coexist, so events from ANY live session are accepted;
	...
	it("updates task usage for the event's session when it has a start result", async () => {
		// Cline Cubed: usage is persisted against the event's OWN session, not the focused
