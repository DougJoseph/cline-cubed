...
export class SdkSessionLifecycle {
	...
	setRunning(isRunning: boolean, sessionId?: string): void {
		// Cline Cubed: set the flag on the TARGET session (default: the focused one).
	...
	/**
	 * Cline Cubed: the live session for `sessionId`, if it is still running in the map.
	...
	async startNewSession(
		...
		// Cline Cubed: a new session does NOT stop any other session — the SDK core runs
	...
	fireAndForgetSend(
		...
		// Cline Cubed: snapshot THIS SEND'S session, not whichever session happens to be focused.
		// Chats run side by side, so the focused session is unrelated to where this send belongs;
		// comparing against it would discard the completion callbacks of every background chat.
		const sessionAtSend = this.sessions.get(sessionId)
		const isSuperseded = (label: string): boolean => {
			// A send is superseded only when ITS OWN session was stopped or replaced.
