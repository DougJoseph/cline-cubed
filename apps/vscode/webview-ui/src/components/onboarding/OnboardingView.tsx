...
import ClineCubedIcon from "@/assets/ClineCubedIcon"
...
const OnboardingViewContent = ({ onboardingModels }: { onboardingModels: OnboardingModelGroup }) => {
	...
	const { openRouterModels, hideSettings, hideAccount, setShowWelcome, dismissOnboardingHere } = useExtensionState()
	...
	const finishOnboarding = useCallback(
		async (updateModelId: boolean, step: number, markCompleted = true) => {
			...
			if (markCompleted) {
				...
				// Cline Cubed: a RE-VIEW (the "Get Started" command) is driven by this surface's
				// own flag, and for an existing user welcomeViewCompleted is ALREADY true — so
				// without clearing the flag here, finishing the flow leaves the screen up and the
				// final Continue looks dead.
	...
	return (
		<div className="fixed inset-0 p-0 flex flex-col w-full">
			<div className="h-full px-5 xs:mx-10 overflow-auto flex flex-col gap-4 items-center justify-center">
				<ClineCubedIcon className="size-16 flex-shrink-0" />
